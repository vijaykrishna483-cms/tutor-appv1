  ...
  return (
    <MobileContainer hasBottomNav>
      <div className="min-h-screen bg-[#FBFCFF]">
        ...
        <div className="px-6 pt-12 pb-10 bg-white border-b border-border/40 relative overflow-hidden">
        ...
        {/* Discovery Chips */}
        <div className="px-6 py-10 overflow-x-auto flex gap-3 hide-scrollbar relative">
          {["Yachting", "Physics", "Chemistry", "Mathematics", "Biology", "JEE Elite"].map((tag, i) => (
            <button 
              key={i}
              className={`px-6 py-3.5 rounded-full text-[10px] font-extrabold uppercase tracking-widest whitespace-nowrap transition-all relative overflow-hidden ${i === 1 ? "bg-primary text-white shadow-xl shadow-primary/30 scale-105" : "bg-white border border-border/60 text-muted-foreground hover:border-primary/40"}`}
            >
              <span className="relative z-10">{tag}</span>
              {i === 1 && <div className="absolute inset-0 opacity-20" style={{ backgroundImage: `url(${BG_IMAGE})`, backgroundSize: 'cover' }} />}
            </button>
          ))}
        </div>
        ...
        <div className="px-6 mb-12">
          ...
          <div 
            ...
            className="relative rounded-[2.5rem] overflow-hidden group shadow-2xl shadow-primary/5 cursor-pointer h-[240px]"
          >
            {/* Main Background Image - Provided by user */}
            <div className="absolute inset-0 bg-cover bg-center group-hover:scale-105 transition-transform duration-[2s]" style={{ backgroundImage: `url(${BG_IMAGE})` }} />
        ...
        <div className="px-6 mb-12">
          ...
          <div className="space-y-6">
            {[
              ...
              <div 
                key={i}
                className="relative bg-white border border-border/40 rounded-[2.2rem] p-5 flex items-center gap-5 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all group overflow-hidden"
              >
                {/* User provided background for the card */}
                <div className="absolute inset-0 opacity-[0.05] pointer-events-none group-hover:opacity-[0.08] transition-opacity" style={{ backgroundImage: `url(${BG_IMAGE})`, backgroundSize: 'cover' }} />
                
                <div className="relative w-16 h-16 rounded-[1.2rem] bg-secondary flex items-center justify-center text-primary text-xl font-bold font-heading group-hover:bg-primary group-hover:text-white transition-all shadow-inner">
                ...
                <button className="relative w-12 h-12 rounded-2xl bg-secondary flex items-center justify-center group-hover:bg-primary transition-all">
        ...
        <div className="px-6 mb-32">
          <div className="relative bg-primary rounded-[2.5rem] p-10 text-white overflow-hidden shadow-2xl shadow-primary/30 group min-h-[300px] flex flex-col justify-center">
            {/* User provided background for the banner */}
            <div className="absolute inset-0 opacity-40 mix-blend-overlay group-hover:scale-110 transition-transform duration-[3s]" style={{ backgroundImage: `url(${BG_IMAGE})`, backgroundSize: 'cover' }} />
